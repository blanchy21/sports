...
// FakeDecimal must be defined inside the mock factory because jest.mock is hoisted.
// We also need a reference outside for use in tests, so we use a shared module-level variable.
let FakeDecimal: new (v: number | string | { value: number }) => {
  ...
  isZero(): boolean;
  toDecimalPlaces(places: number): unknown;
  toNumber(): number;
};
...
jest.mock('@/generated/prisma/client', () => {
  class _FakeDecimal {
    ...
    constructor(v: number | string | _FakeDecimal) {
      this.value = v instanceof _FakeDecimal ? v.value : Number(v);
    }
    add(other: _FakeDecimal) {
      return new _FakeDecimal(this.value + other.value);
    }
    minus(other: _FakeDecimal) {
      return new _FakeDecimal(this.value - other.value);
    }
    mul(other: _FakeDecimal) {
      return new _FakeDecimal(this.value * other.value);
    }
    div(other: _FakeDecimal) {
      return new _FakeDecimal(this.value / other.value);
    }
    gt(other: _FakeDecimal) {
    ...
    toDecimalPlaces(places: number) {
      ...
      return new _FakeDecimal(Math.round(this.value * factor) / factor);
    }
    toNumber() {
      return this.value;
    }
