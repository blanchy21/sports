        ...
        <aside className="hidden bg-background lg:fixed lg:inset-y-0 lg:block lg:w-80 lg:border-r lg:pb-4 lg:pt-[4.5rem]">
          <div className="space-y-2 px-4 py-4">
            {Array.from({ length: 12 }).map((_, i) => (
              <div
                key={i}
                className="h-12 animate-pulse rounded-md bg-sb-turf"
          ...
          <div className="mx-auto max-w-4xl px-6 py-6">
            ...
            <div className="mb-6 flex items-center justify-between">
              <div className="flex items-center space-x-3">
                <div className="h-6 w-6 animate-pulse rounded bg-sb-turf" />
                <div className="h-8 w-40 animate-pulse rounded bg-sb-turf" />
              </div>
            </div>
        ...
        <aside className="hidden bg-background xl:fixed xl:inset-y-0 xl:right-0 xl:block xl:w-[28rem] xl:border-l xl:pb-4 xl:pt-[4.5rem]">
          <div className="space-y-6 px-6 py-4">
            ...
            <div className="space-y-3">
              <div className="h-5 w-32 animate-pulse rounded bg-sb-turf" />
              {Array.from({ length: 3 }).map((_, i) => (
                <div
                  key={i}
                  className="h-20 animate-pulse rounded-lg bg-sb-turf"
            ...
            <div className="space-y-3">
              <div className="h-5 w-36 animate-pulse rounded bg-sb-turf" />
              {Array.from({ length: 4 }).map((_, i) => (
                <div
                  key={i}
                  className="h-8 animate-pulse rounded bg-sb-turf"
                  style={{ animationDelay: `${i * 100}ms` }}
                />
              ))}
            </div>
