...
export const GET = createApiHandler(ROUTE, async (request) => {
  ...
  if (!userId && !username) {
    return validationError('Either userId or username is required');
  }
  ...
  const currentUser = await getAuthenticatedUserFromSession(request as NextRequest);
  ...
  if (currentUser) {
    ...
    if (userIdsToCheck.length > 0) {
      ...
      const followedSet = new Set(existingFollows.map((f: { followedId: string }) => f.followedId));
