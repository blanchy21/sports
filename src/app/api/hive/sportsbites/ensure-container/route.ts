...
export async function POST(request: NextRequest) {
  return withCsrfProtection(request, async () => {
    ...
    // Rate limiting
    const rateLimit = await checkRateLimit(user.userId, RATE_LIMITS.write, 'ensureContainer');
    ...
    try {
      ...
      if (result.success) {
        ...
        ctx.log.info(`Created container for ${dateStr}: tx=${result.transactionId}`);
        return NextResponse.json({
          success: true,
          permlink,
          created: true,
      ...
      if (isDuplicate) {
        createdContainers.add(permlink);
        ctx.log.info(`Container already exists for ${dateStr}`);
        return NextResponse.json({
          success: true,
          permlink,
          alreadyExists: true,
