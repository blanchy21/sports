...
function buildShortMetadata(data: PublishShortData): string {
  const metadata = {
    ...
    format: 'markdown',
    tags: [...SHORTS_CONFIG.DEFAULT_TAGS, ...(data.sportCategory ? [data.sportCategory] : [])],
...
export async function fetchShorts(options: {
  ...
  try {
    ...
    const result = await makeHiveApiCall<unknown[]>('condenser_api', 'get_content_replies', [
      SHORTS_CONFIG.PARENT_AUTHOR,
      SHORTS_CONFIG.PARENT_PERMLINK,
    ]);
