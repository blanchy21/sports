...
interface MatchThreadFeedProps {
  eventId: string;
  isLive: boolean;
  className?: string;
  ...
  const loadBites = useCallback(
    async (loadMore = false) => {
      ...
      try {
        ...
        const response = await fetch(`/api/match-threads/${eventId}/bites?${params.toString()}`);
        if (!response.ok) throw new Error(`Failed to fetch: ${response.status}`);

        const result: SportsbiteApiResponse = await response.json();
        if (!result.success) throw new Error(result.error || 'Failed to fetch thread bites');

        setBites((prev) => {
          const merged = loadMore ? [...prev, ...result.sportsbites] : result.sportsbites;
        ...
        logger.error('Error loading thread bites', 'MatchThreadFeed', err);
        setError(err instanceof Error ? err.message : 'Failed to load thread bites');
