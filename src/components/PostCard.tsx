...
const PostCardComponent: React.FC<PostCardProps> = ({ post, className }) => {
  ...
  const authorUsername = isHivePost
    ? post.author
          ...
          <div className="min-w-0 flex-1">
            <div className="flex items-center space-x-2">
              ...
              <span className="text-sm text-muted-foreground">
                {formatDate(
                  isHivePost ? new Date(post.created) : post.publishedAt || post.createdAt
                )}
              </span>
              <span className="text-muted-foreground">•</span>
              <span className="text-sm text-muted-foreground">
            ...
            {!isHivePost && post.sport && (
              <div className="mt-1 flex items-center space-x-1">
                ...
                <span className="text-xs text-muted-foreground">{post.sport.name}</span>
              </div>
            )}
            {isHivePost && post.sportCategory && (
              <div className="mt-1 flex items-center space-x-1">
                <MapPin className="h-3 w-3 text-muted-foreground" />
                <span className="text-xs text-muted-foreground">{post.sportCategory}</span>
              </div>
            )}
