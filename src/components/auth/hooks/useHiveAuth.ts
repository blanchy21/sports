...
export const useHiveAuth = () => {
  ...
  const updateHiveField = useCallback((field: keyof HiveAuthState, value: string | boolean | string[] | DiscoveredAccount[]) => {
  ...
  const handleAiohaLogin = useCallback(async (provider: string) => {
  ...
  const handleAccountSelect = useCallback(async (account: DiscoveredAccount) => {
    try {
